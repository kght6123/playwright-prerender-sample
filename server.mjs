...
const prerenderUrlList = ['/', '/index.html']
...
prerenderUrlList.forEach((prerenderUrl) => {
  router.get(prerenderUrl, async (req, res, next) => {
    console.log(`----- prerender ----- ${url + prerenderUrl}`)
    // if (prerender.isCache(url + prerenderUrl)) {
    const { html, ttRenderMs } = await prerender.ssr(
      url + prerenderUrl,
      wsEndpoint,
      waitForSelector
    )
    ...
    // } else {
    //   proxy(req, res, next, url + prerenderUrl) // FIXME: なぜか、302リダイレクトになる（Tomcat側でどこかエラーになってる？）
    // }
    console.log(`----- prerender ----- ${url + prerenderUrl}`)
  })
})
...
router.get('/cron/update_cache', async (req, res) => {
...
app.use('/', router)
app.listen(58080, () =>
  console.log('Server started http://localhost:58080/ Press Ctrl+C to quit')
)
